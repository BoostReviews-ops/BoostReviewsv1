# Turning BoostReviewsAI live

Everything below assumes the app is deployed on Vercel at a domain we call `APP_DOMAIN`
(temporary: `https://<project>.vercel.app`, production: `https://app.boostreviewsai.com`).
...
3. **OAuth consent screen** — External; app name BoostReviewsAI; support email; add the
   ...
   the scope is sensitive (privacy policy URL on boostreviewsai.com, homepage, justification video).
...
5. **Environment variables** (Vercel → Project → Settings → Environment Variables, Production + Preview):
   ...
   ```
6. **Test with your own profile** — sign in, Settings → Integrations → *Connect Google*, approve the
   consent screen. The callback stores tokens in `google_connections` and picks your first
   location. Then hit `/api/cron/sync-reviews` once with `Authorization: Bearer $CRON_SECRET`
   (Vercel Cron runs it every 6 hours per `vercel.json`) and confirm reviews appear in `/app/reviews`.
...
2. Create one product "BoostReviewsAI software" with two recurring prices: $89/month and $801/year.
   Create three one-time prices for setup: Connect $129, Pro $179.99, Website Upgrade $399.
...
4. Env vars:
   ...
   STRIPE_PRICE_SETUP_CONNECT=price_...
   STRIPE_PRICE_SETUP_PRO=price_...
   STRIPE_PRICE_SETUP_WEBSITE=price_...
...
6. Test: Billing → Upgrade uses Stripe Checkout with card `4242 4242 4242 4242`; the webhook
   handler upserts the `subscriptions` table (keyed by `metadata.org_id` set at checkout).
...
3. Authentication → Providers: enable Email (magic link) and Google; set Site URL to
   `https://APP_DOMAIN` and add `https://APP_DOMAIN/api/auth/callback` to the redirect URL allow-list.
   The login page sends magic links from the browser; `/api/auth/callback` exchanges the code for a session.
   `src/proxy.ts` then gates `/app` — signed-in users see their own data, prospects with the
   demo cookie (set by `/demo`) keep seeing Royal Massage & Spa.
...
## Custom domain (app.boostreviewsai.com)

1. Vercel → Project → Settings → Domains → Add `app.boostreviewsai.com`.
...
4. Set `NEXT_PUBLIC_APP_URL=https://app.boostreviewsai.com`, update `GOOGLE_REDIRECT_URI`, the
   Google authorized redirect URI, and the Stripe webhook URL to the new domain, then redeploy.

## NFC production setup

- URL to program on every card/stand: `https://app.boostreviewsai.com/r/<slug>` — HTTPS, no
...
- **Never change after distribution:** the domain `app.boostreviewsai.com`, the `/r/` path, and
  each card's slug. Never delete a `review_cards` row — pause it instead.

## Onboarding my first real business

1. Deploy to production, connect `app.boostreviewsai.com`, set Supabase + Stripe + Google vars.
...
6. Program the NFC card/stand with `https://app.boostreviewsai.com/r/<slug>`, test on two phones.
