  ...
  return (
    <>
      ...
      <section className="mt-6">
        <p className="mb-3 text-[12px] font-bold uppercase tracking-wider text-ink-subtle">Plans</p>
            ...
            return (
              <div key={p.id} className={cn("card relative flex flex-col p-5", p.highlight && "border-brand-400 ring-2 ring-brand-100")}>
                ...
                <Button className="mt-5" variant={isCurrent ? "outline" : p.highlight ? "primary" : "secondary"} disabled={isCurrent} onClick={() => setConfirm(p.id)} full>
                  {isCurrent ? "Current plan" : p.priceMonthly > current.priceMonthly ? "Upgrade" : "Switch"}
                </Button>
              </div>
            );
          })}
      ...
      <Modal
        ...
        description={confirm ? `${formatCurrency(getPlan(confirm).priceMonthly)}/month, prorated from today.` : ""}
        size="sm"
        footer={
